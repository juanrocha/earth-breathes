...
- `tiling_quality_flags.R` can be used to remove low quality values. One option is to remove them from the final product. Another, more intersting, is to account for it before and after the analysis, to distinguish pixels where quality is high versus places where quality may induce errors. 
- New versions of `stars` and `terra` working well with hdf files. 
- First replicate Nielja's paper 1.
- Calculate how bat it is using only high quality values. How many missing values are created or that need to be interpolated. I could either interpolate myself, or use the estimate from MODIS, but being clear that risk of false positives are induced due to interpolation increase correlation.

**260220:**

- If you want to plot the full map, you need to extract a tiff file per tile with summary statistics.
For example number of EWS. Then stich them together and use a downsampling technique to
plot so the files are manageable (GDAL or stars will do).
- It would be great to study the effect of data quality on the EWS. The problem is
I will need to work again with the hdf files and extract quality flags. Perhaps use `zarr`
and optimize for time series instead of netcdf? -- Do it in Runa.
...
cmake -DCMAKE_DISABLE_FIND_PACKAGE_Arrow=ON \
      ...
      -DHDF4_szip_LIBRARY_RELEASE=/opt/homebrew/lib/libsz.dylib \
      -DHDF4_xdr_LIBRARY_RELEASE=/opt/homebrew/lib/libsz.dylib \
      ...
      -DPROJ_LIBRARY_RELEASE=/opt/homebrew/opt/proj/lib/libproj.dylib \
      -DSWIG_EXECUTABLE=/opt/homebrew/opt/swig/bin/swig \
      -DSWIG_DIR:PATH=/opt/homebrew/opt/swig \
      -DGDAL_SET_INSTALL_RELATIVE_RPATH=ON \
      -DCMAKE_INSTALL_PREFIX=/opt/homebrew/Cellar/gdal/gdal-3.9.1_from-source/ \
...
```{bash eval = FALSE}
      -DHDF4_LIBRARY:FILEPATH=/opt/homebrew/Cellar/hdf4/lib/libhdf4.dylib \ 
...
```{bash engine.opts='-l'}
#https://forum.posit.co/t/unique-paths-between-rstudio-notebook-and-rstudio-terminal/7936
echo $PATH
gdalinfo --version
```
...
```
## new path
/opt/homebrew/opt/gdal/3.9.0/lib:/opt/homebrew/opt/proj/bin:/opt/homebrew/opt/jpeg/bin:/Users/juanrocha/.juliaup/bin:/opt/homebrew/opt/gdal/3.9.0/lib:/opt/homebrew/opt/proj/bin:/opt/homebrew/opt/jpeg/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/Library/Frameworks/Python.framework/Versions/3.12/bin:/usr/local/bin:/System/Cryptexes/App/usr/bin:/usr/bin:/bin:/usr/sbin:/sbin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin:/Library/Apple/usr/bin:/opt/homebrew/Cellar/gdal/3.9.0/lib:/opt/homebrew/opt/proj/bin:/opt/homebrew/opt/jpeg/bin:/Users/juanrocha/.juliaup/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/Library/Frameworks/Python.framework/Versions/3.12/bin:/opt/homebrew/Cellar/gdal/3.9.0/bin:/opt/homebrew/bin/proj:/opt/homebrew/Cellar/gdal-3.8.5/lib/bin:/var/folders/jm/d3760lfd491gkrb4_gll_n_w0000gn/T/RtmpyZAL9t/rstudio/terminal:/var/folders/jm/d3760lfd491gkrb4_gll_n_w0000gn/T/RtmpUPvm3b/rstudio/terminal:/var/folders/jm/d3760lfd491gkrb4_gll_n_w0000gn/T/Rtmpp5lRkj/rstudio/terminal:/Users/juanrocha/Applications/quarto/bin:/Library/TeX/texbin:/usr/texbin:/Applications/RStudio.app/Contents/Resources/app/quarto/bin:/Applications/RStudio.app/Contents/Resources/app/bin/postback
